...
// LaneConfig 每条线路配置
// +kubebuilder:validation:XValidation:rule="!(self.ispType in ['ChinaTelecom','ChinaUnicom','ChinaMobile','ChinaTelecom_L2','ChinaUnicom_L2','ChinaMobile_L2']) || has(self.bandwidthPackageId)",message="single-ISP lanes (ChinaTelecom/ChinaUnicom/ChinaMobile and their L2 variants) require spec.lanes[].bandwidthPackageId; NLB cannot directly attach single-ISP EIPs (use a CommonBandwidthPackage with matching ISP instead)"
type LaneConfig struct {
	// Name lane名称，用于CR命名
	Name string `json:"name"`
	// ISPType 线路类型: BGP, BGP_PRO, ChinaTelecom, ChinaUnicom, ChinaMobile
	ISPType string `json:"ispType"`
	// Bandwidth 独立 EIP 的带宽峰值，单位 Mbps（仅 PayByTraffic 独立 EIP 路径生效）。
	// 与 BandwidthPackageId 互斥：填了 BandwidthPackageId 时此字段被忽略。
	// 未填时使用云端 PayByTraffic 默认 5 Mbps 峰值。
	// +optional
	Bandwidth string `json:"bandwidth,omitempty"`
	// BandwidthPackageId 共享带宽包 ID（CommonBandwidthPackage）。
	// 填了之后 NLB 自动建 PayByTraffic EIP 并加入此带宽包，实际计费走带宽包。
	// 单线 ISP（ChinaTelecom/Unicom/Mobile）场景必须填此字段（NLB 直接拒绝单线 EIP）。
	...
	// 仅独立 EIP 路径生效（不填 BandwidthPackageId 时）。
	// +optional
